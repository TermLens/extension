...
if (typeof window.termLensUi === 'undefined') {
  window.termLensUi = (function () {
    ...
    function showResultsModal(data) {
      ...
      const evalKey = (c) => (c?.evaluation || '').toLowerCase();
        ...
        <div class="tos-distribution" role="list" aria-label="조항 평가 분포">
          ${['good', 'neutral', 'bad']
            .map((type) => {
              ...
              return `
                <div class="tos-distribution-item ${type}" role="listitem" data-filter="${type}">
        ...
        <div class="tos-analyzer-modal">
          ...
          <div class="tos-analyzer-body">
            ...
            <div class="tos-clauses-section">
              <h4>주요 조항 요약</h4>
              <ul class="tos-clauses-list"></ul>
            </div>
          </div>
        </div>`;
      document.getElementById('close-btn').onclick = removeModal;

      const listEl = m.querySelector('.tos-clauses-list');
      const distItems = Array.from(m.querySelectorAll('.tos-distribution-item'));
      const filterState = { value: 'all' };

      const buildList = (items) => (items.length
        ? items
            .map((c, idx) => {
            ...
            .join('')
        : '<li class="tos-clause-item empty">해당 평가에 속하는 조항이 없습니다.</li>');

      const bindReasonButtons = () => {
      ...
      const setActiveFilterUI = () => {
        distItems.forEach((el) => {
          const key = el.dataset.filter;
          el.classList.toggle('active', key === filterState.value);
        });
      };

      const renderClauses = () => {
        const filtered = filterState.value === 'all'
          ? clauses
          : clauses.filter((c) => evalKey(c) === filterState.value);
      ...
      distItems.forEach((el) => {
        el.addEventListener('click', () => {
          const key = el.dataset.filter;
          filterState.value = filterState.value === key ? 'all' : key;
          setActiveFilterUI();
          renderClauses();
        });
      });
